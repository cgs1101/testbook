...
def order_pay(request):
	...
	#和支付宝进行交互
	alipay = AliPay(
		appid = '2016090900464054',#应用id
		app_notify_url=None,#默认回调url
		app_private_key_path=os.path.join(settings.BASE_DIR,'order/app_private_key.pem'),
		alipay_public_key_path=os.path.join(settings.BASE_DIR,'order/alipay_public_key.pem'),#支付宝的公钥,验证支付宝回传消息使用,不是自己的公钥
		sign_type='RSA2',#RSA或PSA2
		debug=True,#默认是False
	)
...
def check_pay(request):
	...
	while True:
		...
		if code == '10000' and result.get('trade_status') == 'TRADE_SUCCESS':
			...
			order.save()
			#返回数据
			return JsonResponse({'code':3,'message':'支付成功'})
